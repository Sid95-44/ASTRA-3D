...
export function getOrbitalPosition(
  ...
  return {
    x: real.x * visualScale,
    y: real.y * visualScale,
    z: real.z * visualScale,
  };
}
...
export function getHohmannTransferDays(departure: Body, destination: Body) {
  if (
    ...
    departure.semiMajorAxisAU === 0 ||
    destination.semiMajorAxisAU === 0
  )
    return 0;
...
export function formatCompactDistance(distanceKm: number) {
  if (distanceKm >= 1_000_000_000)
    return `${(distanceKm / 1_000_000_000).toFixed(2)} billion km`;
...
export function getMoonPositionAU(
  moon: Moon,
  bodies: Body[],
  simulatedDay: number
): Vector3Like {
...
export function getCelestialPositionAU(
  object: CelestialRecord,
  bodies: Body[],
  moons: Moon[],
  simulatedDay: number
): Vector3Like {
  return "parentId" in object
    ? getMoonPositionAU(object, bodies, simulatedDay)
    : getOrbitalPositionAU(object, simulatedDay);
}

export function getCelestialName(id: string, bodies: Body[], moons: Moon[]) {
  return (
    bodies.find(body => body.id === id)?.name ??
    moons.find(moon => moon.id === id)?.name ??
    id
  );
}

export function getDistanceBetweenObjectsKm(
  firstId: string,
  secondId: string,
  bodies: Body[],
  moons: Moon[],
...
export function getApproximateMissionDays(
  ...
  if (fromMoon || toMoon)
    return fromId === "earth" || toId === "earth" ? 5 : 18;
  return 30;
}
